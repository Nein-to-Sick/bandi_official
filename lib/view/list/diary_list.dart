...
class _DiaryListState extends State<DiaryList> {
  final int _limit = 4;
  List<Map<String, dynamic>> _groupedDiaries = [];
  bool _hasMore = true;
  ...
  Future<void> _loadMoreData() async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      _processNewDiaries(querySnapshot.docs);
      setState(() {
  ...
  void _processNewDiaries(List<QueryDocumentSnapshot> newDocs) {
    String lastDisplayedMonth = _groupedDiaries.isNotEmpty && _groupedDiaries.last['isHeader'] == false
        ? DateFormat('yyyy년 M월').format((_groupedDiaries.last['createdAt'] as DateTime))
        : '';
    ...
    for (var doc in newDocs) {
      ...
      _groupedDiaries.add({'isHeader': false, 'data': diaryData, 'createdAt': createdAt});
    }
  }
      ...
      itemBuilder: (context, index) {
        if (index < _groupedDiaries.length) {
          ...
          if (item['isHeader']) {
            return Padding(
              ...
              child: Center(
                child: Container(
                  ...
                  child: Padding(
                    padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 31),
                    child: Text(
                      item['header'],
            ...
            String combinedEmotions = (diaryData['emotion'] as List<dynamic>).join(', ');
          ...
          return const Padding(
            ...
            child: Center(
              child: SizedBox(width: 40,child: MyFireFlyProgressbar(loadingText: '')),
            ),
          );
        } else {
          return Center(child: Text('끝', style: BandiFont.headlineMedium(context)?.copyWith(
            color: BandiColor.neutralColor60(context),
          )));
        }
      },
